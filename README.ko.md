...
monitor-agent는 **Anthropic API를 전혀 호출하지 않습니다**. Claude Code가 이미 디스크에 남겨둔 JSONL transcript를 읽어서 시각화만 할 뿐이므로:

| 리소스 | 사용량 |
|--------|--------|
| **Claude 토큰 / API 비용** | **0** (외부 호출 없음, 오로지 로컬 파일 읽기) |
| **네트워크** | localhost SSE만 사용, 인터넷 트래픽 0 |
| **CPU** | 아이들 상태 <0.1%, 프롬프트 발생 순간만 JSONL 파싱 (평균 10~30ms) |
| **메모리** | ~50~80MB (Node.js 기본 + 오프셋 캐시 + usage-index.json) |
| **디스크** | `cache/usage-index.json` 수 MB (30일 누적 기준) + offsets.json 수 KB |
| **의존성** | Zero (`npm install` 없음, Node 내장 모듈만) |
